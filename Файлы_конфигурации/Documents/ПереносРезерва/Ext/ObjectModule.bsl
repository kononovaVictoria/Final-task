...
//Движения по регистру РезервированиеТоваров
	...
	Если СтатусРезерва = Перечисления.СтатусыРезерва.Действующий Тогда
		...
		Если ЗначениеЗаполнено(ОбъектОснование)  Тогда
			Если ОбъектОснование.СтатусРезерва<>Перечисления.СтатусыРезерва.Действующий Тогда
				Отказ = Истина;
				Сообщить("Резерв не может быть перенесен, потому что его статус <<"+ОбъектОснование.СтатусРезерва+">>. Для перенова резерва статус должен быть <<Действующий>>.");
				Возврат;	
			КонецЕсли;
		Иначе 
			Отказ = Истина;
		...
		|	РезервированиеТоваровОстатки.КоличествоРезерваОстаток КАК КоличествоРезерваОстаток
		|ИЗ
		...
		Если Отказ = Ложь Тогда
			...
			ДокОбъект.СтатусРезерва    = Перечисления.СтатусыРезерва.Перенесен;
			ДокОбъект.ДокументПереноса = Ссылка;
			ДокОбъект.Записать();
			
			СтатусРезерва              = Перечисления.СтатусыРезерва.Перенесен;
			
			//создаем новый документ резерва
			ДокРезерв                  = Документы.РезервированиеТовара.СоздатьДокумент();
			ДокРезерв.Дата             = ТекущаяДата();
			ДокРезерв.ОбъектОснование  = Ссылка;
			ДокРезерв.ВидЦен           = ВидЦен;
			ДокРезерв.Договор          = Договор;
			ДокРезерв.Комментарий      = "Перенос резерва";
			ДокРезерв.Контрагент       = Контрагент;
			ДокРезерв.Организация      = Организация;
			ДокРезерв.Покупатель       = Покупатель;
			ДокРезерв.Склад            = Склад;
			ДокРезерв.СтатусРезерва    = Перечисления.СтатусыРезерва.Действующий;
			ДокРезерв.СуммаДокумента   = СуммаДокумента;
			Для Каждого ТекСтрокаТаблица Из Таблица Цикл
				НоваяСтрока                             = ДокРезерв.Таблица.Добавить();
...
//Ввод на основании Документа РезервированиеТовара 
